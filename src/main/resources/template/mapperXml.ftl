...
<mapper namespace="${packageName}.mapper.${modelName}Mapper">
    ...
    <sql id="columns">
        <#list data as var>
            <#if var_index != 0>,</#if>x1.${var.columeName}<#if var.mod!='default' && var.attrName!='id'>,${var.modModule2}.${var.modColValue} as ${var.modValueAlias}</#if>
             <#if  var.attrName=='id'>
    ...
    <sql id="middles">
        ...
        <#list data as var>
            <#if var.mod!='default' && var.attrName!='id'>
                left join (select * from ${var.modTable} where is_deleted=0) ${var.modModule2}  on x1.${var.columeName}=${var.modModule2}.${var.modColKey}
            </#if>
            <#if  var.attrName=='id'>
                <#list var.mulTableVoList as vl >
    ...
    <sql id="conditions">
        <#list data as var>
            <#if (var.dataType=='char' || var.dataType=='varchar' || var.dataType=='text' || var.dataType=='tinytext') && var.attrName!='id' && var.attrName!='deptId'  >
              ...
              and ${var.modModule2}.${var.modColValue} like CONCAT('%',${r"#"}{vo.${var.modValueAlias}},'%')
                    </if>
                </#if>
            </#if>
            <#if  var.attrName=='id'>
